...
Reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and this fence is a confused-deputy defense rather than identity. Connection applies [browser token authentication](2026-08-24-browser-token-authentication.md) or the explicit [trusted-network socket policy](2026-09-03-trusted-network-browser-authentication.md) after the fence. The fence itself does not inspect peer socket addresses: binding expresses reachability, `trustedHosts` names accepted authorities, and socket facts belong only to the separate authentication decision.
...
- A non-loopback composition must trust its serving authorities or requests are refused, then satisfy browser authentication or an explicit direct socket rule. The shipped CLI supports both an authenticated all-interface bind and a separately enabled private-network bypass; `--trusted-host` only extends the Host/Origin fence and grants no identity.
- Clients must label POST bodies `application/json` (ours always did; raw-fetch tests gained the header).
- Host and Origin remain request-routing evidence only. The process token and signed cookie establish the normal browser identity; destination-and-source socket rules can establish the separate explicit trusted-network identity.
