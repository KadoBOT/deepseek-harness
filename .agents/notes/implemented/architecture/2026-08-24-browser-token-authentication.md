...
`dsh-client-connection` authenticates the complete Host API before dispatch. By default, every API Proxy method, Remote unary call, generic Connection channel, and Remote WebSocket stream requires the same browser session; endpoint ownership and method names do not alter authority. The explicit [trusted-network decision](2026-09-03-trusted-network-browser-authentication.md) adds one route-wide alternative for matching direct socket peers without creating method-specific tiers. The existing Host/Origin checks run first and retain their DNS-rebinding and cross-site-request role, returning 403 when they fail. A trusted Host without either accepted identity receives 401. The browser-trust rules remain owned by the [carrier-level browser trust decision](2026-07-28-api-browser-trust-boundary.md).
...
The shipped CLI supports an explicit `--host 0.0.0.0` bind. Browser authentication remains active in that mode unless `--allow-unauthenticated-network` derives a matching direct socket rule. Neither mode implies TLS, forwarding-header interpretation, or proxy identity.

## Verification

Unit coverage pins process-token retention across Connection reloads, one secret load per activation, synchronous verification without credential-provider reads, cookie attributes, HMAC and payload validation, authority and lifetime checks, record deletion taking effect on the next activation, invalid durable records, and cleanup of obsolete token URLs backed by valid cookies. Host transport suites pin 401/403 behavior for generic RPC, Typert Remote HTTP, exact Fetch routes, and WebSocket upgrade paths; trusted-network suites separately pin the explicit socket exception without changing endpoint coverage. The frontend real-composition test boots credentials, Connection, webserver, and static serving through Loader and proves token exchange before index reads while static assets remain public. Packed-worker tests prove portable cookie encoding and worker-local retry for both authentication and trust rejection. A real-CLI test starts `dsh web` twice on one port with a temporary `DSH_HOME`, proves that forged `Host: localhost` is unauthenticated, calls `settings/describe` with the exchanged cookie, observes a new process token, and reuses the old cookie after restart.

## Alternatives considered

**Determine privileged callers solely from the TCP peer address.** A direct peer address can identify a local forwarding process rather than the browser user and requires proxy policy to answer who the original caller was. The later trusted-network decision admits only explicit destination-and-source rules for direct private peers; it does not turn peer addresses or forwarding headers into a universal identity.
...
Possession of the browser cookie authorizes the complete tool-capable Host API, matching the authority the Web application exposes after Session creation. A peer admitted by an explicit trusted-network rule receives the same authority. `Host` does not grant a higher method tier, and a method migration between API Proxy and Typert Remote cannot change its caller set.
...
The decision partially supersedes the authentication deferral and unauthenticated non-loopback consequences in the [browser trust note](2026-07-28-api-browser-trust-boundary.md). The [trusted-network decision](2026-09-03-trusted-network-browser-authentication.md) partially supersedes this note's uniform-cookie rule for an explicit private-peer mode. All three remain active because their cookie, browser-trust, and network-risk rules retain independent future decision value.
