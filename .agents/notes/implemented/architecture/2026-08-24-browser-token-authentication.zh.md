...
`dsh-client-connection` 在分发前认证完整 Host API。默认情况下，每个 API Proxy 方法、Remote 一元调用、通用 Connection channel 和 Remote WebSocket stream 都要求同一个浏览器会话；endpoint 所有权与方法名称不改变 authority。显式的[可信网络决策](2026-09-03-trusted-network-browser-authentication.zh.md)为匹配的直接套接字对等端添加一条 route 级替代路径，而不会建立按方法区分的层级。既有 Host/Origin 校验先执行，继续负责 DNS rebinding 和跨站请求防御，失败时返回 403。Host 可信但不具备任一受接受身份时返回 401。浏览器信任规则仍由[载体级浏览器信任决策](2026-07-28-api-browser-trust-boundary.zh.md)持有。
...
随附 CLI 支持显式 `--host 0.0.0.0` 绑定。除非 `--allow-unauthenticated-network` 派生出匹配的直接套接字规则，否则该模式仍使用浏览器认证。两种模式都不代表具备 TLS、转发 header 解释或代理身份。

## 验证

单元覆盖 Connection 重载时保留进程令牌、每次激活只加载一次密钥、无需读取凭据提供方的同步校验、cookie 属性、HMAC 与 payload 校验、authority 与有效期校验、记录删除在下一次激活时生效、无效持久记录，以及用有效 cookie 清理过时令牌 URL。Host 传输套件固定通用 RPC、Typert Remote HTTP、精确 Fetch 路由和 WebSocket upgrade 路径上的 401/403 行为；可信网络套件单独固定显式套接字例外，而不改变 endpoint 覆盖。frontend 真实组合测试经 Loader 启动 credentials、Connection、webserver 与静态服务，证明读取 index 前完成令牌交换，同时静态资产仍公开。打包 worker 测试证明 cookie 编码可移植，并覆盖认证与信任拒绝后的 worker 本地重试。真实 CLI 测试在临时 `DSH_HOME` 上用同一端口两次启动 `dsh web`，证明伪造 `Host: localhost` 仍未认证，以交换所得 cookie 调用 `settings/describe`，观测新的进程令牌，并在重启后复用旧 cookie。

## 曾考虑的替代方案

**只从 TCP 对端地址判定特权调用者。** 直接对端地址可能只识别本地转发进程，而非浏览器用户，并要求代理策略回答原始调用者是谁。后续可信网络决策只允许为直接私有对等端显式配置目标加源地址规则；它不会把对端地址或转发标头变成通用身份。
...
持有浏览器 cookie 就能调用完整的工具型 Host API，这与 Web 应用在创建 Session 后暴露的 authority 一致。显式可信网络规则接纳的对等端会获得相同 authority。`Host` 不授予更高的方法层级，方法在 API Proxy 与 Typert Remote 之间迁移也不会改变调用者集合。
...
本决策部分取代[浏览器信任说明](2026-07-28-api-browser-trust-boundary.zh.md)中的认证延期与未认证非 loopback 后果。[可信网络决策](2026-09-03-trusted-network-browser-authentication.zh.md)又在显式私有对等端模式下部分取代本说明的统一 cookie 规则。三份说明都会保持 active，因为其中的 cookie、浏览器信任与网络风险规则分别保有独立的未来决策价值。
