---
description: "settings、凭据配置与浏览器授权界面的 Host Remote owner。"
...
`@deepseek-ai/dsh-api-settings-controller` 为浏览器配置界面提供生成的 `ctx.remote.settings`、`ctx.remote.credentials` 与 `ctx.remote.authorization` namespace。它返回脱敏的 settings、凭据元数据与浏览器授权进度，支持 settings 与凭据写入而不返回密钥值，并在 Host 桌面打开由 provider 持有的 settings 或 Agent preset 位置。provider 缺失时，namespace 仍会注册，并返回可操作的配置错误。
...
请把本包作为 Loader entry 挂载到提供浏览器配置的 profile 中。本 entry 不依赖 provider 是否存在而注册三个 namespace，因此缺少 provider 会在调用时产生具名配置错误。它生成的 descriptor 进入严格 Typert registry，而 settings、凭据与授权 Definition 仍是普通 Cordis Service，自身不承担任何 wire 义务。
...
`authorization.list()` 列出已注册的 flow 与脱敏凭据元数据。`authorization.begin()` 启动一个 flow 并返回不透明的 attempt id；浏览器轮询 `read()`，用 `respond()` 回答当前 prompt，也可以用 `cancel()` 或 `disconnect()` 终止它。每个 flow 只保留最新 attempt，prompt 回答会立即消费，授权超时由 `authTimeoutMs` 控制。
...
| `authTimeoutMs` | 10 分钟 | 一个浏览器授权 flow 能运行的最长时间 |
