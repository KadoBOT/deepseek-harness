...
提供方会提供带品牌类型的稳定 id、自动模式（`first-prompt` 或 `all-prompts`）和 `generate(request)`。请求携带活跃会话、截至一次固定修订的所有符合条件消息、可用时当前已记录的主请求路由，以及取消信号。结果包含非空标题、该请求中互不重复且有序的来源消息 seq，以及生成该标题时使用的可选提供方／模型路由。brief 提供方还可以追加一条仅写入日志的 `session/summary` 事件——先于其伴随的标题被接受——携带一句话摘要与相同的来源 seq。服务会在标题持久保存前进行规范化和验证；投影键 `title` 与 `summary` 声明于本包的 [`src/types.ts`](src/types.ts)，把两个值送达客户端。
...
回退、已接受的提供方修订与摘要事件都不会向主 agent 请求增加 token。可选提供方的独立辅助请求由对应提供方包的文档说明。
