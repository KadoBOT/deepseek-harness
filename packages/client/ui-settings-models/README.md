...
Providers that advertise an OAuth method show a generic **Connect account** control. The host owns the sign-in flow and sends only browser-safe notices, URLs, device codes, and short-lived prompts to this card; secret prompt answers are sent once and are never rendered back or retained by the page. A pending flow is polled without overlapping requests and is cancelled when the card leaves the page. A completed grant keeps its provider row visible and offers **Enable** when the settings profile is dormant. Enabling removes only that profile's user API-key override, and refuses when a deployment-level key would remain in force; unrelated API-key references and defaults remain untouched. **Disconnect account** removes only the matching grant.
