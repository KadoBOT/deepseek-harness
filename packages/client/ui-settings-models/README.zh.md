...
声明 OAuth 方法的提供方会显示通用的**连接账户**控件。登录流程由宿主负责，这张卡片只接收浏览器安全的提示、URL、设备码与短时提示；机密提示的答案只发送一次，页面既不会再次渲染也不会保留。卡片离开页面时，未完成的流程会在不重叠请求的轮询中被取消。授权完成后，即使设置 profile 处于休眠状态，提供方行仍会保留并显示「启用」。启用只移除该 profile 的用户层 API 密钥覆盖；若部署层密钥仍会生效则拒绝操作。其他 API 密钥引用和默认值都保持不变。「断开账户」只会移除相应的授权记录。
