...
The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; in-process compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The Host half owns the sole `/api` route, Fetch bridge, browser and direct socket-peer authentication, Host/Origin checks, and exact `GET`/`HEAD` route registry. Typert Gateway claims generated Remote endpoints, feature packages register non-JSON responses such as Session-log downloads, and unclaimed requests return 404. Loopback hostname classification remains package-internal to the browser-facing Client state.
...
Every Host RPC method and WebSocket stream uses one route-wide authentication policy; endpoint names do not create privileged tiers. The normal path requires a browser session. Each process mints a random launch token, and `dsh-web-app` prints and opens the ordinary root URL with `?token=...`; `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /`, writes an authority-bound signed cookie, and redirects to clean `/`. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. The HTTP carrier accepts no query token outside the root exchange and no Authorization-header token.

`unauthenticatedNetworkRules` adds an explicit direct-peer path to the same policy. Each rule pairs the local IPv4 destination with one source subnet; Connection normalizes plain and IPv4-mapped socket addresses and bypasses the cookie only when both addresses match one rule and the remote address is not loopback. Missing or malformed socket facts and unmatched peers use browser authentication. Forwarding headers are never read. `dsh-web-app` derives only contained RFC 1918 interface subnets and Tailscale's `100.64.0.0/10` range when its explicit CLI flag is enabled; loopback and ordinary all-interface launches remain tokenized.
...
Before either authentication path accepts a Host API request or WebSocket upgrade, `src/api-request-trust.ts` checks that its `Host` is loopback or matches a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. A matching network peer's index request passes the same checks before HTML is served. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md), [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md), and [trusted-network browser authentication](../../../.agents/notes/implemented/architecture/2026-09-03-trusted-network-browser-authentication.md).
...
- **Neither browser nor network authentication adds TLS** — the cookie is not marked `Secure`, and a socket-matched peer receives complete Host authority over plaintext HTTP.
